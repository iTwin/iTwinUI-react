...
const ignoredWarnings = ['AssetsOverSizeLimitWarning', 'EntrypointsOverSizeLimitWarning', 'NoAsyncChunksWarning'];

class WebpackFailOnWarningsPlugin {
  apply(compiler) {
    compiler.hooks.done.tap('FailOnWarnings', (stats) => {
      const warnings = stats.compilation.warnings.filter((warning) => !ignoredWarnings.includes(warning.name));
